  ...
  void (async () => {
    try {
      ...
      if (haveOid === desktopHead) {
        updateClientFromRequest(request, { recentlySyncedString: lingo('GitSync/AlreadyUpToDate') });
      ...
      updateClientFromRequest(request, { recentlySyncedString: lingo('GitSync/BundleSent') });
